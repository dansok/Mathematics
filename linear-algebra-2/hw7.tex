...
Thus $||A^*A|| = ||A||^2$.
\\$\mathbf{(b)}$ By Theorem $12$, $||Q(Ax)|| = ||Ax||$, and $P$ (and $Q$) is invertible, and $P^{-1}$ is an isometry. Let $E = \{ x \in X : ||x|| = 1 \}$. Let $F = P^{-1}(E)$. Since $P$ is invertible, $P(F) = P(P^{-1}(E)) = E$. We claim $E = F-\ \ y \in F \implies Py \in E$, and $||y|| = ||Py|| = 1$ by definition of $E$. Thus $y \in E$. $x \in E \implies Px \in E$ since $||Px|| = ||x|| = 1$. Thus $x = P^{-1}(Px) \in F$. Therefore $E = F$ as claimed. Finally,
