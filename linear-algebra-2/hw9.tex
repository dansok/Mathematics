...
\end{question}
\begin{proof}
Suppose $\{f(s_j)\}_{1 \le j \le m}$ are linearly independent and $f(t) \in span\{f(s_j)\}$ for some $t \ne s_j$ for any $1 \le j \le m$. Then $f_i(t) = \sum_{j=1}^m c_j f_i(s_j)$, contradicting the fact that .
\end{proof}
