...
\\$rank(f(t)^T) = rank(f(t)) = m$, meaning there are also only $m$ linearly independent rows in $f(t)$. That is, $\{f_i(s_j)\}_{1 \le i \le n}$ must be a linearly dependent set for some $s_j$, contradicting the fact that $\{f_i\}_{1 \le i \le n}$ are linearly independent.
\end{proof}
