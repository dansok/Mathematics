...
\\$rank(f(t)^T) = rank(f(t)) = m$, meaning there are also only $m$ linearly independent rows in $f(t)$. Say $\sum_{j=1}^m f_i(s_j) = \sum_{k \ne i} \sum_{j=1}^m a_k \cdot f_k(s_j) \iff f_i(s_j) = \sum_{k \ne i} a_k \cdot f_k(s_j)$ for all $s_j$. That is, $\{f_i(s_j)\}_{1 \le i \le n}$ is a linearly dependent set, contradicting the fact that $\{f_i\}_{1 \le i \le n}$ are linearly independent.
\end{proof}
